                                        ...
                                        const filtered = derivaciones.filter(d => {
                                            ...
                                            if (statusFilter) {
                                                if (statusFilter === 'PENDIENTE') {
                                                    matchesStatus = ['SCHEDULED', 'PENDING', 'PENDIENTE'].includes(d.status) || (!['COMPLETED', 'FINALIZADO', 'RECEIVED', 'EN_PROCESO', 'CANCELLED', 'ANULADO'].includes(d.status));
                                                } else if (statusFilter === 'EN_PROCESO') {
                                                    matchesStatus = ['RECEIVED', 'EN_PROCESO'].includes(d.status);
                                                } else if (statusFilter === 'FINALIZADO') {
                                                    matchesStatus = ['COMPLETED', 'FINALIZADO'].includes(d.status);
                                                } else if (statusFilter === 'ANULADO') {
                                                    matchesStatus = ['CANCELLED', 'ANULADO'].includes(d.status);
                                                }
                                            }
