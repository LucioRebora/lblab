...
export default function PreciosDerivantesPage() {
    const { data: session, status } = useSession();
    const router = useRouter();

    const [prices, setPrices] = useState<AnalysisPrice[]>([]);
    const [nbuValue, setNbuValue] = useState<number>(529);
    const [validity, setValidity] = useState<string>("Febrero 2026");
    ...
    useEffect(() => {
        if (status === "unauthenticated") {
            ...
            const canAccess = isStaff || u?.canAccessDerivaciones;
    ...
    const handleSaveGlobal = async (key: string, value: string) => {
        if (key === "NBU_VALUE") setIsSavingNbu(true);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const body = editingItem
                ? { ...formData, id: editingItem.id, category: "DERIVANTE" }
                : { ...formData, category: "DERIVANTE" };
            ...
            <main className="flex-grow overflow-y-auto">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none text-black">
                                    {isStaff ? (isAdmin ? "Admin" : "Secretaría") : "Derivante"}
                                </p>
                            </div>
                ...
                <div className="p-8 space-y-8">
                    <div className="flex justify-between items-center">
                        <div>
                            <h1 className="text-3xl font-black text-gray-900 uppercase tracking-tight">Lista Precios Derivantes</h1>
                            <p className="text-gray-500 text-sm font-medium">Gestión de NBU y valores de análisis para convenios.</p>
                        </div>
                        {isStaff && (
                            <button
                                onClick={() => {
                                ...
                                className="bg-primary-burgundy text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg shadow-primary-burgundy/20 hover:scale-105 transition-all flex items-center gap-2"
                            >
                                <Plus size={18} />
                                Nuevo Análisis
                            </button>
                    ...
                    {isStaff && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
                                <div className="flex items-center gap-3 text-primary-burgundy">
                                    <Activity size={20} />
                                    <h3 className="font-black text-xs uppercase tracking-widest">Valor NBU Global</h3>
                                </div>
                                <div className="flex gap-4">
                                    <div className="relative flex-grow">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-gray-50 border-none rounded-2xl py-3 pl-10 pr-4 text-lg font-black text-gray-800 focus:ring-2 focus:ring-primary-burgundy outline-none"
                                        />
                                    </div>
                                    <button
                                        onClick={() => handleSaveGlobal("NBU_VALUE", nbuValue.toString())}
                                ...
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest pl-2">
                                    Este valor se utiliza para calcular el precio final ($) de todos los análisis.
                                </p>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
                                <div className="flex items-center gap-3 text-blue-500">
                                    ...
                                    <h3 className="font-black text-xs uppercase tracking-widest">Vigencia de Lista</h3>
                                </div>
                                <div className="flex gap-4">
                                    <input
                                        type="text"
                                    ...
                                    />
                                    <button
                                        onClick={() => handleSaveGlobal("VIGENCIA", validity)}
                                ...
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest pl-2">
                                    Fecha informativa de entrada en vigencia que aparecerá en los reportes.
                                </p>
                            </div>
                        </div>
                    )}
                                    ...
                                    {prices.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="px-8 py-20 text-center text-gray-400 font-bold uppercase tracking-widest text-xs italic">
                                                {searchQuery ? "No se encontraron análisis que coincidan." : "No hay análisis cargados."}
                                            </td>
                                        </tr>
                                    ) : (
                                        prices
                        ...
                        >
                            <div className="bg-primary-burgundy p-8 text-center relative">
                                ...
                                <h3 className="text-xl font-black text-white uppercase tracking-tight">
                                    {editingItem ? "Editar Análisis" : "Nuevo Análisis"}
                                </h3>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-8 space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 text-sm font-bold focus:ring-2 focus:ring-primary-burgundy outline-none text-black uppercase"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 text-sm font-bold focus:ring-2 focus:ring-primary-burgundy outline-none text-black"
                                        placeholder="Ej: 10"
                                    />
                                </div>
                                ...
                                <div className="flex gap-4 pt-4">
                                    ...
                                    <button
                                        ...
                                        className="flex-[2] bg-primary-burgundy text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-primary-burgundy/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                                    >
                                        <Save size={16} />
